...
bool point_reached(sensors_event_t event, int check) {
  switch (current_state) {
    case HEEL_LIFT_STATE: // if we are in the heel lift state
      ...
      if (check == 0){
          ...
          if (!(targeted_bounds(event))){ 
            ...
            //ln("Knee has gone out of bounds, will pull back.");
            //return true;
          }else{
      ...
      if (check == 1){
          ...
          if (targeted_bounds(event)) {
            //ln("Foot_IMU goal reached, now on EXTENSION_FORWARD_STATE_PART_2");
    ...
    // so the motors don't stop when they should (missed)
      ...
      if (targeted_bounds(event)) {
        //ln("Knee_IMU goal reached, now on FOLLOW_THROUGH_STATE");
    ...
    case FOLLOW_THROUGH_STATE:    //ADDED BY ETHAN. CHECK.
      ...
      if (targeted_bounds(event)) {  
        //ln("Foot_IMU goal reached, now on EQUILIBRIUM_STATE_KNEE_IMU");
     ...
     if (check == 0) { // knee
        ...
        if (targeted_bounds(event)) {
        //ln("Knee_IMU goal reached, now checking on foot IMU");
//        current_state = EQUILIBRIUM_STATE_KNEE_IMU; // current state is now a different range
        return true;
        }
      }
    ...
    case EQUILIBRIUM_STATE_KNEE_IMU:
      ...
      if (targeted_bounds(event)) {
        //ln("Knee_IMU goal reached, now on EQUILIBRIUM_STATE_FOOT_IMU");
...
// return_equilibrium_knee():
// Flexes the knee backwards at a specified speed. 
void return_equilibrium_knee(uint8_t motor_direction_f, uint8_t motor_direction_b, int speed) {
  //ln("Return to beginning (knee).");
  // specifies the speed/direction pattern of active tensile elements
  increase_motor_speed(ILIOPSOAS_MOTOR_NUMBER, motor_direction_f, motor_direction_b, speed); 
}

// return_equilibrium_foot():
// Flexes the knee backwards at a specified speed. 
void return_equilibrium_foot(uint8_t motor_direction_f, uint8_t motor_direction_b, int speed) {
  //ln("Return to beginning (foot).");
  // specifies the speed/direction pattern of active tensile elements
  increase_motor_speed(HAMSTRING_MOTOR_NUMBER, motor_direction_f, motor_direction_b, speed); 
}
...
void stop_motion() {
  ...
  delay(100); // discharge all current through motors 100 before
}
...
void setup() {
  ...
  if(!lower_leg_tracker.begin()) {
    ...
    //("Ooops, no BNO055 (foot) detected ... Check your wiring or I2C ADDR!"); 
    while(1);
  }
  
  lower_leg_tracker.setExtCrystalUse(true);

  if(!knee_tracker.begin()) {
    // There was a problem detecting the BNO055 ... check your connections
    //("Ooops, no BNO055 (knee) detected ... Check your wiring or I2C ADDR!"); 
    while(1);
  }
  
  knee_tracker.setExtCrystalUse(true);
  ...
  if (Serial.available() > 0) {
    ...
    switch (command) {
      case 's': //s for start
        ...
        break;
      case 'x': // x for exit
        //ln("Stopping all motion.");
        stop_motion();
        break;
      case 'p'://print current gryoscopic value
        //("Event_Foot = ");
        //ln(event_foot.orientation.x, 4);
        //("Event_Knee = ");
        //ln(event_knee.orientation.x, 4);
        break;
...
//  Constantly print out the gyroscopic readings from the BNO055
  ...
  //old_event = event_foot; // for direction_of_movement, not currently being used.
    ...
    ms = millis()-starting;
    ("time: ");
    Serial.println(ms/1000);    //prints timerrr since program started
    delay(9);
   //removed because inconsistent printings     
