  ...
  switch(motor_number){
    case 1:
      ...
      for(i=0;i<motor_speed;i++){
       ...
       b_hip_motor->setSpeed(i*.4); //rear hip motor releases simultaneously with front x2 to increase speed
    ...
    case 3:
      ...
      Serial.print("increasing motor speed");
      for(i=0;i<motor_speed;i++){
       hamstring_motor->setSpeed(i); //new just need to be i, 255 is max value and it will reach there afterwards
       delay(5);
      }
      break;
...
//(int motor_number, uint8_t motor_direction_f, uint8_t motor_direction_b, int motor_speed)
  increase_motor_speed(3, motor_direction_f,motor_direction_b,255);
  delay(distance*fact);
